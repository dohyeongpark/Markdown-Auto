...
- **웹훅 시크릿의 레포별 분리.** 현재 `GITHUB_WEBHOOK_SECRET`은 서버 전체에 하나뿐인 공유 값이라, 이 값을
  아는 사람은 자기 소유가 아닌 다른 레포로도 서명된 가짜 push payload를 만들어 보낼 수 있다. 지금은 본인만
  쓰는 구조라 문제 없지만, 다른 사람의 GitHub 계정 소유 레포에도 이 서비스를 웹훅으로 연결해주기로 하면
  (2026-07-26 논의) `app/auth_store.py`의 repo API 키처럼 레포별 웹훅 시크릿을 발급·검증하는 구조로 바꿔야
  한다. 혼자만 쓰는 동안은 우선순위 낮음(YAGNI).
